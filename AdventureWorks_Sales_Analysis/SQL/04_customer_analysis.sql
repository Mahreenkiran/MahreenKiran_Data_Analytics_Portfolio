...
/*Question 15: Annual sales performance
How did total revenue, total orders, total units sold and average order value change by year?*/

WITH UNITSINFO AS
(
SELECT YEAR(OrderDate) as YearOfSales,
SUM(ORDERQTY) TotalUnitsSold
FROM AdventureWorks2025.Sales.SalesOrderHeader AS H
INNER JOIN AdventureWorks2025.Sales.SalesOrderDetail AS O
...
U.TotalUnitsSold AS TotalUnitsSold
FROM AdventureWorks2025.Sales.SalesOrderHeader AS H
INNER JOIN UNITSINFO AS U
ON YEAR(H.OrderDate)=U.YearOfSales
GROUP BY YEAR(OrderDate),U.TotalUnitsSold
ORDER BY YEAR(OrderDate) ;

-------------------------------------------------------------------                         
-------------------------------------------------------------------
/*Optional Question 16: Customer concentration
What percentage of total company revenue was generated by the top 10 customers?*/

WITH TopTenCustomerRevenue AS
(
    SELECT
        SUM(CustomerRevenue) AS TopTenRevenue
    FROM
    (
        SELECT TOP 10
            CustomerID,
            SUM(SubTotal) AS CustomerRevenue
...
SELECT
    CAST(t.TopTenRevenue AS DECIMAL(18,2)) AS TopTenCustomerRevenue,
    CAST(c.TotalRevenue AS DECIMAL(18,2)) AS TotalCompanyRevenue,
    CAST(
        t.TopTenRevenue / NULLIF(c.TotalRevenue, 0) * 100
        AS DECIMAL(18,2)
    ) AS TopTenRevenueContributionPercentage
FROM TopTenCustomerRevenue AS t
CROSS JOIN TotalCompanyRevenue AS c;
